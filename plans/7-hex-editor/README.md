...
| **detail fades, it never pops** | W0 | the ground fades to the sky across a band ending at or before the draw distance, and the sky is **one value** used for both the clear and the fog target. **Control:** give the fog a different colour from the clear and a ring appears at the horizon — two derivations of one value, wearing a colour. ⚠ **This band is the LOD seam** (user): later it cross-fades detailed geometry into coarse, so both must derive from the same field and the band must stay wider than a chunk. **The far layer's intended form is recorded, NOT built** (user, 2026-07-25: *"eventually — do not build yet — we will get a height texture with xy modifiers to show the outlines and colors of less detailed features"*): a height texture plus xy modifiers carrying outline and colour, i.e. the distant world as a **sampled field** rather than emitted geometry. Two things that follows from, worth holding before it is built — it must be **derived from the same field** as the near geometry (a separately authored far layer is a second source, and the fade band is exactly where a disagreement becomes visible), and a texture is a **layer-2 derivation**, so `L3` forbids persisting it |
