...
**Every phase is now built.** `A2b` was the last, and `A3` needed no rule at all.

### ✅ `A2b` — THE CORRIDOR DID NOT MOVE AT ALL, AND ONE HALF OF THAT WAS A BUG
...
✅ **AND THE BAND'S CEILING IS BUILT TOO — the corridor follows.** *Sub-surface runs take
a slope, not a lift.* When a stroke moves the ground, the layer under it moves by the same
delta, so the cover holds at **12 all along the run** where it used to grow 12 → 24 without
bound. ⚠ Moved by the ground's own delta rather than by measuring the band and steering for
it: the separation `F1` cares about is unchanged **by construction**, so this can neither
breach the ceiling nor be refused for closing on it.

⚠ **AND THE ROW THAT ASSERTED THE NOT-BUILT STATE IS WHAT CAUGHT THE CHANGE**, which is
what it was written for — it went red at *"the corridor floor moved to 0; if that is now
intended, this row is the one to change on purpose."* It now asserts the cover at **every**
cell of the run, not only at the peak: a rule that moved the centre and left the flanks
would hold at `q=0` and put a step in the roof.

✅ **THE NEGATIVE CONTROL HOLDS BY CONSTRUCTION.** *A corridor under a BUILDING must still
move rigidly.* A house's floor is fabric, fabric is `taken`, and the falloff loop this band
lives in has already skipped every taken cell — `A1`'s rigid path owns them and moves the
whole column. Measured and pinned: the house rose 3 and its cellar rose 3.

⚠ **OPEN QUESTION 3 IS ANSWERED BY WHAT A FLOOR IS.** *Cover or gradient?* — they cannot
fight here, because `FLOOR_MAT`'s `tr_slope` is `SLOPE_FREE`. A corridor has no gradient
limit of its own, so it follows the ground exactly and the cover is held rather than
floating. ⚠ The question becomes live the day a sub-surface run carries a limited material,
and the answer the plan reaches for — *the gradient wins and the cover floats* — is then one
line: clamp the follow and let the cover open up.
...
| **`A2b`** — sub-surface runs take a slope, not a lift | M | a corridor keeps its cover within a band and its gradient within its limit | ✅ **built** — the cover holds in both directions, and open questions 3 and 4 are answered. See below |
...
| ~~`A2b`~~ | ✅ both ends of the band: a hollow over a corridor is refused by `F1` and **reported**, and a raise **takes the corridor along** so the cover holds at 12 along the whole run. The gradient half is vacuous — `FLOOR_MAT` is `SLOPE_FREE` | *a corridor's cover is bounded and its gradient limited* | ✅ two: bare ground takes the same hollow in silence, and a cellar under a **building** rides rigidly with it |
