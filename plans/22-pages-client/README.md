...
**Next:** the last piece of **`T1`** — the **verb bar as a QUERY**. `verbs_here()` was deleted at
`T1a.2` for having no consumer, and its two blockers are now one: the world still has to be
threaded to `client_verb_specs` (its call sites hold only `Client`, though the client holds
`st.cache`), and a type verb still has **no key** — *"possibly even unique keys"* — which is a
declaration change. Until then the bar draws the compiled-in verbs and a declared one is
reachable by script and by wire but not by keyboard.
