...
⛔ **AND `D2` IS BLOCKED, WHICH `D1`'s OWN LOG FOUND — [`probe/d2`](../../probe/d2/README.md).**
...
walk in, and **they cannot** — `session_open_kind` marks **one** hex edge and the boundary a
walker crosses is **two**, so the editor's own doorway is drawn open and stays shut. The `inside`
mode is unreachable by walking, in every driver.

**Next:** **`D1a`** — an opening marks the edges its own width covers — then `D2`. **`T1`** is
unblocked and independent of both.
...
| **`D1a.2`** — an opening marks the edges its own WIDTH covers, not one | S | ⚠ the walker must reach the far wall, and a NARROW opening must still mark fewer edges than a wide one — otherwise the fix is *mark everything* | ⏭ **UNBLOCKED by `D1a.1`** — a ring has a centreline now, so `run_point_near` has something to project onto and the five `opening.loft` fixtures that refused will have an opening whose centre is on the wall. ⛔ **The first attempt is recorded below because its failure is what found `D1a.1`:** `open_span` plus a shape-first `session_open_kind` made **5 of `opening.loft`'s 18** tests refuse with *no wall here to open* — because `fence_disc` registers **no RUN**, so `run_point_near` leaves the opening's centre at the author's CELL CENTRE, 0.866 from every edge midpoint against a half of 0.65. ⚠ **`opening_cuts` — the renderer's own inside-test — uses the same plain distance**, so by the drawing's own rule no point of that boundary is inside that opening: the store has said `DOOR` there since `open_ahead` was written and **the picture has no hole in it.** So the step also has to decide what an opening means on a wall no run describes — give `fence_disc` a run, or fall back to the edge it was found on. ⏭ Effort raised from `S`; the two candidates are in the probe. Everything below is what still holds: | `session_open_kind` hardcodes `open_ahead(w, a, DOOR_MAT, 1)`; the cell a walker crosses carries **two** wall edges, so the marked one is `DOOR` and the walker meets the other. Measured through the tick's own `walk_proxy` + `walk_to`: **z = −2.26** with the editor's own cut, **z = 5.24** with the cell's other edge cut too, **z = 6** with no house — the control that says the instrument can report movement at all. ⛔ **So the `inside` mode is unreachable by walking, in every driver.** ⚠ The cause is the split `gesture.loft` names itself — *"the edges are the INDEX; the opening is the TRUTH"* — and the walk reads only the index: a `op_half 0.65` doorway is **1.3 wu** and a hex edge is 0.87 |
