...
| **`D2a`** — the roof reaches the walls it belongs to | S | ⚠ **not *the flip moved*** — the gable's four eave corners must meet the four runs, and the ridge moves with them | ⏭ **UNBLOCKED, and `D2p` found its CAUSE rather than its symptom.** The two hypotheses for the flip were separated by printing `mode_at`'s own inputs: `feet` is **40 at every station**, so *the author is on top of the wall* is refuted, and what is left is that `plan_over` is **false at the wall's own line**. `place_house` files both, and they disagree — the runs at x −3.85…5.58 · z −2.25…5.25 against the gable's eave corners at x −3.46…5.20 · z −1.96…4.96, **short by 0.385 and 0.286 on every side**. ⛔ **So the drawn roof does not reach its own walls**, and with `D1a.3`'s masonry at ±0.433 about its run the eave lands *inside the wall's thickness*. ⚠ **It is `D1a.3`'s class one gesture over**: `roof_plan_of` copies the footprint's CELL COUNTS and `plan_hw` reads them back as `n · HEX_LEN / 2`, while the walls come from `footprint_walls` mitring on the footprint's own boundary — two conversions of one footprint into one rectangle. ⚠ **And the cost is stated before the step rather than found during it**: `roof_ridge_y` is `eave + pitch · plan_hd`, so a roof that reaches its walls is wider AND taller, and every gate that photographs a house moves ⛔ **AND IT IS NOT A COLLAR OF STAIRCASE — MEASURED, [`probe/d2` § 8](../../probe/d2/README.md)**: the house meshed over its four chunks is **5340/3892 with the plan against 5493/4042 without**, and **all 27 roof cells are suppressed** while **2 of them sit beyond the drawn gable** — so the band round the house has its cells removed and nothing drawn over it. ⛔ **The tree already holds TWO tests of this one rectangle, half a hex apart**: `roof_plan_covers` (the mesher's, per CELL) inflates by `HEX_LEN·0.5` and `roof_plan_over` (`shelter_at`'s, per POINT) does not — each right about what it asks, and the gable drawn only to `hw` is what leaves the band. ⚠ **And the two of twenty-seven is a floating-point equality**, cell centres at x 5.196 against a rectangle edge of 5.196. ⚠ **The correction is not one constant** — 0.385 in x against 0.286 in z — which is what says it must come from the footprint rather than from a fudge. ⛔ **The probe's own first run printed a full set of plausible numbers while aimed nowhere**: `chunk_meshes_all` takes CHUNK INDICES, and 25 chunks of defaulted ground all read 448/384, so it concluded *the plan saves 0* |
