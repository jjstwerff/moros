...
| **`K2b`**–**`K3`** — convert the rest, then drop the key spelling | M | a converted script and its original build the same world **and the same session** | ⏭ **Five keys left** — `E Z X C B` (`R`, `Y`, `T`, `J`, `K`, `V` done; `Q` used by none) |
