...
| **`D1a`** — an opening marks the edges its own WIDTH covers, not one | S | ⚠ the walker must reach the far wall, and a NARROW opening must still mark fewer edges than a wide one — otherwise the fix is *mark everything* | ⛔ **BLOCKS `D2`, and it is measured rather than suspected — [`probe/d2`](../../probe/d2/README.md).** `session_open_kind` hardcodes `open_ahead(w, a, DOOR_MAT, 1)`; the cell a walker crosses carries **two** wall edges, so the marked one is `DOOR` and the walker meets the other. Measured through the tick's own `walk_proxy` + `walk_to`: **z = −2.26** with the editor's own cut, **z = 5.24** with the cell's other edge cut too, **z = 6** with no house — the control that says the instrument can report movement at all. ⛔ **So the `inside` mode is unreachable by walking, in every driver.** ⚠ The cause is the split `gesture.loft` names itself — *"the edges are the INDEX; the opening is the TRUTH"* — and the walk reads only the index: a `op_half 0.65` doorway is **1.3 wu** and a hex edge is 0.87 |
