...
⛔ **AND `D2`'s BINDING TABLE IS REFUTED BY ITS OWN PREMISE — `D2p`, 2026-08-17.** The mode and
the opening verb disagree **in both directions**, and the line runs through the wall: the near
wall is filed at `z −2.25`, the mode flips at **`z −2.00`**, so **an author standing on their own
wall reads `outside`**. Over 25 stations the design's table would refuse **8** where the gesture
works and grant **4** where there is no wall. ⛔ **And no drawing of the line reaches the corpus** —
a roof plan is filed by `verb place` alone, and **16 of the 18 openings are cut into free-standing
`verb run` walls** with no `place` in the script: *a wall does not need a roof.* ✅ **The control is
what makes that evidence rather than argument**: `storey` over a bare meadow builds a floor on 19
cells of open grass and `place` from inside a room stamps a second whole house, with nothing in the
tree refusing either.

✅ **AND `D2a`'s OWN SENTENCE IS MEASURED, NOT INFERRED — 2026-08-17.** `D2a.1`/`D2a.2` recorded
what was *built*; re-running `probe/d2/gate.loft` is what says it worked. The mode's flip moved
from **0.25 inside the wall's run to 0.43 outside it**, and 0.43 is `wall_band()/2` — the
masonry's own outer face, so *an author standing on their own wall reads `outside`* is false at
every station in the walled band. ⛔ **And the disagreement did NOT close, which is the point**:
the mode is a continuous rectangle and the verb is a hex-shaped cell sweep reaching four units,
so no placement of the rectangle could ever have made them agree. **`D2p`'s split of `D2` stands,
now measured from the far side of the fix.** ⚠ The old *refuse 8 · grant 4* is not comparable
with today's **14 · 18** — row B's window was drawn from the hex-quantised `store WALL` column
and undercounted at both ends; it is row A's window now.

⛔ **AND THE HOUSE THE EDITOR PLACES HAS TWO OF ITS FOUR WALLS MISSING — found on the way,
pre-existing, controlled.** `place_house` reports **`84 wall edges`** and the store holds
**23**: the near wall is complete at 12 and the left at 9, while the **right has 4 of ~9** and
the **far wall is its two corner posts and nothing between**. So the editor cannot cut a door or
a window in the far wall of any house it places — measured as the gesture, not as a count.
⚠ **Asked at twelve heights before it was called missing**, and ⚠ **a worktree at `3e3ac22`
gives the same 23 edges**, so it is not a `D2a` regression. It is invisible because the mesher
draws walls from the **runs**, which are all four and correct — *the picture is right and the
store is wrong.* ✅ **And the cause is located: it is the ROOF, not the stamping.** All four
sides stamp completely (22+20+22+20 = the 84 the sentence prints); `roof_over` then merges a
band **from the floor** to the ridge over every footprint cell and takes the walls with it —
**far run 24 → 4, near run 24 → 24, one line apart in the same measurement.** ⏭ **`D2c`.**

✅ **AND `D2c` IS BUILT — the four walls all stand, 2026-08-17.** It was the **roof**, not the
stamping: all four sides stamp completely and `roof_over` then merged a band from the floor to
the ridge and took two of them with it. The fix is three lines and a subtraction — the band is
the roof only, because `place_house` had already laid the floor. The store after a roof is
**46 → 84 sightings**, matching `place_house`'s own sentence for the first time, and a door goes
in any of the four walls.

✅ **AND `D2b` IS HALF BUILT AND HALF REFUTED, 2026-08-18.** `place` refuses a house whose
footprint overlaps a filed plan — and writes nothing doing it. ⛔ **The storey half is dropped
because its own verify contradicts itself**: all three live storeys (`deck`, `cellar`,
`threshold`) run over bare raised ground with `verb place` count **zero**, so *the meadow must
refuse* and *the corpus's storeys must still build* are the same case. **A storey is a terrain
gesture in this editor, not a building one.**

✅ **AND `D2c.1` CLOSED THE LOOSE END `D2c` LEFT** — `floor_h` is out of `roof_over`'s
signature, because the band starts at the eave now and `rp.rp_eave` is the only height the
function needs. A parameter every caller passes and nothing reads is worse than dead code: it
tells the next reader the roof is seated on that number when it is not.

✅ **AND `T1a` CLEARED ITS PATH, 2026-08-18** — the storey height had three names in three
packages and has one now (`STOREY_H`), demonstrated by a sabotage that moved the ridge 23 → 24
where before the merge it moved nothing at all.

✅ **AND THE FORMAT IS SETTLED AND BUILT — `T1a.1`, 2026-08-18.** A house type is a **palette
entry** (plan 21's, on the owner's call), the store carries the payload without knowing what a
...
✅ **AND A DECLARED VERB TRAVELS NOW — `T1d`, 2026-08-20.** `55:<verb>` is `press_verb` on the
socket, so `verb portcullis` does over the wire what it has done headless since `T1c`. ⛔ **The
step's own risk was the floor it removed**: dropping `script.mjs`'s *no verb 'X'* refusal let a
typo exit 0, so the question moved to the server's `no gesture for X` reply — one authority for
*is this a verb*, and `verb plce` still exits 101.

✅ **AND `T1` IS FINISHED — `T1e`, 2026-08-20.** `verbs_here()` is back with the two consumers it
was deleted for lacking: the verb bar draws the castle's own verb on the key it declared, and the
keyboard performs it. **The verb table is data now** — *which keys does this editor have* is a
query answered from the world the author is standing in, and adding a type touches no code from
the declaration to the glyph.

⏭ **What `T1` leaves**, measured rather than promised: **the page cannot declare anything.** `T1c`
gave the runner a `declare` line and the server `54:`; local mode got neither, because a
declaration is not a gesture and the page has no text input — so the browser proof runs under a
FIXTURE that patches one call into the page's boot. A page that could open a document declaring a
type would need `world.hxw` to carry one, and that file is baked into the `--html` build.
...
| ✅ **`D2p`** — the premise: is the MODE the right authority for whether a verb exists? | XS | ⚠ **it must be able to say the design is RIGHT** — a probe with only one reachable verdict is not an instrument, so the control is a verb with no precondition of its own | **DONE 2026-08-17**, [`probe/d2` § 7](../../probe/d2/README.md). ⛔ **THE MODE AND THE OPENING VERB DISAGREE IN BOTH DIRECTIONS, AND THE LINE RUNS THROUGH THE WALL**: the near wall's run is filed at `z −2.25`, the cells carrying wall span `−4.00 … −1.25`, and the mode flips at **`z −2.00`** — so **an author standing on their own wall reads `outside`**. Over 25 stations the design's table would **REFUSE 8 where the gesture works** and **GRANT 4 where there is no wall**, while the verb cuts at the same `(0,−2.25)` from every working station: one authority is stable where the verb is used and the other is not. ⛔ **AND NO DRAWING OF THE LINE REACHES THE CORPUS.** Row D gave the alternative its own chance — *the table may be right and its LINE wrong* — and inflating the roof plan one cell moves the flip **−2.00 → −3.50**, which does fix `house.keys`'s two-modes-one-comment. It rescues nothing: a roof plan is filed by `verb place` alone, and **16 of the corpus's 18 openings are cut into free-standing `verb run` walls with no `verb place` in the script at all.** ⚠ **A wall does not need a roof.** ✅ **AND THE CONTROL FIRES, WHICH IS WHAT MAKES THE REST EVIDENCE**: `storey` over a bare meadow builds a floor on **19 cells of open grass** and `place` from inside a room stamps **a second whole house — 84 writes, roofs 1 → 2** — with nothing in the tree refusing either. ⚠ **But what those two want is *is there a building here*, and the mode answers *am I under a roof*** — the same word `outside` for a meadow and for the wall the corpus performs both its storeys from. ⛔ **AND `raise` FROM INSIDE DOES NOT MOVE THE GROUND UNDER THE BUILDING** at either facing — `PEAK_AHEAD` is 10 hexes against a 7.5 wu house, so **0 of 13 moved cells are inside the footprint** and the `inside` row's stated reason is not reproducible with the editor's own house. ⚠ **Two instruments were wrong before the subject was**: the `w_tau` control measured 2 writes on a refusal that should have had none (written downstream of the second house, which had put a wall in the room), and the `raise` row read one cell ahead and printed `40 → 40`, which reads as *nothing moved*. ⛔ **Side finding, independent of `D2`: `place_house` answers `ak_ok: false` after 57 writes and 4 filed runs** — *the walls stand but the roof was refused* — which is the shape `storey_here`'s own comment names as the thing a refusal must never mean |
| ✅ **`D2a.1`** — the roof's cells come from its plan, not from a second body | S | ⚠ **byte-identical, and measured before it was claimed** — if the world moves at all, the two paths were not agreeing and the next phase would be moving two things at once | **DONE 2026-08-17.** `S6b`'s own comment called it agreement — *"both take the ridge height from the same `eave + pitch * hd`"* — but that is the same FORMULA from two bodies (`hex_draw::draw_roof` for the cells, `roof_plan_y` for the gable), which holds only while both read the same rectangle. `roof_over` takes the filed `RoofPlan` and writes each cell from `roof_plan_y`; `place_house` builds it one line earlier and files the object the cells came from. ✅ **`probe/d2/roofcells.loft` is what said it could be byte-identical**: the stored heights were already `floor(roof_plan_y / unit)` at all 27 cells — −0.299/−0.499/−0.699, pure truncation, no shape. `make parts` byte-identical · `make headless-same` rc 0 · `probe/k3d` **31 unmoved** · hex_editor 580. ⚠ **`hip_steps` is refused rather than ignored** — every caller passes 0, and a parameter silently dropped is how a capability comes back as a bug report |
| ✅ **`D2a.2`** — the rectangle is the outline its own walls enclose | S | ⚠ **not *the roof got bigger*** — it must land on the wall's face at EVERY size, and the nominal rectangle must be shown to miss | **DONE 2026-08-17.** ⛔ **THE RULE COULD NOT BE READ OFF ONE HOUSE, AND THE SIGN IS WHY**: over 20 (wid, dep) pairs the nominal rectangle **overhangs** the walls at dep 3 (−0.348) and 5 (−0.580) and **falls short** at 4 (+0.286) and 6 (+0.054), alternating along u by the parity of `wid` too — so no constant outset could have fixed it, and §7's *the plan is 0.385 short* was a single instance wearing a rule's clothes. `RoofPlan` carries `rp_hw`/`rp_hd` from `footprint_walls`' mitred outline plus `wall_band()/2`, and the six extent readers use them. ✅ **Re-measured: exactly −0.433 at all twenty pairs on both axes** — the eave lands on the wall's outer face at every size. ⛔ **And the edge-by-edge version of the claim is FALSE, measured before it was dropped**: *every stamped edge MIDPOINT is under the roof* reds at 2 of 34 on a 4×3, and the excess is **only ever over u and at most 0.197** — the stamped boundary is a zigzag and the roof is a straight rectangle, which is the whole of `S6b`. So what is asserted is *every column the house roofed is under its own plan*, over nine sizes, **with the nominal rectangle as the control that must miss** — without it the row is satisfied by any roof big enough and has never been red. ✅ **`probe/k3d` moved exactly the 9 scripts that place a house, and every script that places one**, every changed line the ridge `21 → 23` (`pitch 0.7 × 0.719 / 0.25`), with no cell count, wall-edge count, τ, chunk count or other sentence moving. ⚠ **And one line that is not a height**: `house.keys`' two openings — the two `D1` found reading DIFFERENT modes at stations its own comment describes identically — are **both `inside`** now. `D1`'s question closes as a consequence of the geometry rather than as a rule about modes  ⛔ **AND THE GATE SUITE CANNOT SEE THIS CHANGE — `44 PASS` IS 44 GATES THAT COULD NOT.** `GATE_JOBS=1 make gate`, all 49 serially: **44 PASS, 5 FAIL**, and every failure is loft#950 — `cache` `camera_indoors` `cellar_ceiling` `client_mesh` `deck_soffit`, each with the recorded `parts -1` `cam false` `sky 1` signature. ⚠ **`deck_soffit` is a fifth STATE's list did not have** and it was **controlled rather than assumed**: a worktree at the pre-`D2a` commit fails it too, on the same row, with `mesh soffit = 342 vertices PASS` identical on both sides. ⛔ **Every gate that renders a house is among the five that are down**; `part_mode`, the one that compares the roof-plan registry, does it as a **save-and-restore round trip** and is blind by construction to a roof that changed size on both sides; and **no gate asserts the ridge at all**. So what measures this step is `roof_plan.loft`, and what is genuinely unverified is the **picture** — the `--html` client traps before it draws, so neither a gate nor `make probe-demo` can photograph a house today. ⚠ **And `make lib-test` failed once, transiently, in a way that reads as a broken package**: `hex_mesh` came back 10 files of parse errors (*"Library `hex_edge` not found"* against `hex_way`'s own source) — two worktrees, at the pre-`D2a` commit and at HEAD, both answer **73 passed**. Two `loft` builds at once, and the failure mode is a resolution error pointing at a registry package. · `make lib-test` **1872 → 1874 both backends** · `make fast` rc 0 · `make parts` byte-identical · `make headless-same` rc 0 |
| ✅ **`D2c`** — `place_house` stamps all four of its walls | S | ⚠ **not the count in the sentence** — that says `84` today against 23 in the store, so it is the thing being fixed and cannot be the check. The gesture is: an opening cut from inside each of the four walls, facing it, and the far one must go from refused to cut | **DONE 2026-08-17.** ⛔ **IT WAS THE ROOF, AND THE STAMPING WAS INNOCENT** — all four sides stamp completely (22+20+22+20 = the 84 the sentence prints) and every write survives; then `roof_over` merged a band from **the floor** to the ridge over every footprint cell and took the walls with it. Stage by stage: **floor 0 → walls 84 → roof 46**, far run **24 → 4** with the near run **24 → 24** in the same measurement, one line apart. ✅ **The fix is three lines and a SUBTRACTION**: the band is `[eave_u, hu]` carrying only the roof, because `place_house` already laid the floor with `ground_set(…, FLOOR_MAT)` four lines earlier — the band's floor cell was a **second assertion of a fact that already had a home**, and `band_column` replaces everything between `lo` and `hi`, taking `sv_wall_*` with it. ⚠ **The placement was decided by a probe, not by taste**: the other two `world_merge_band_as` callers (the stencil hut, `hex_part`'s stamp) put the wall fields INTO their band, so the API is not at fault and this caller was — *a roof is not the authority on a wall*. ⚠ **loft had said so on every run** — `advice[omitted-field-zero]` named `h_wall_nw`/`h_wall_ne`/`h_wall_e` on the deleted literal. ⚠ **And the mesher's own comment is the control on the subtraction**: *"`place_house` writes its floor into the ground layer as `FLOOR_MAT`"*, so the floor's single home is that pass. `tests/house_walls.loft` **3 of 5 seen red first**, with the near wall and the roof/floor rows green as controls; the four walls are asserted **separately, never as a total**, because 24+24+24+12 reads as healthy and the near wall survived the defect by luck of ownership. ⚠ **One red row was my own instrument** — the ridge guard used `ground_h`, which answers the GROUND and read 40 under a finished roof; it uses `tests/roof.loft`'s own `roof_at` body now. The store after a roof is **46 → 84 sightings**, matching `place_house`'s sentence for the first time, and the far wall goes **REFUSED → cut at (0,5.25)** · `make lib-test` **1874 → 1879 both backends** (hex_editor 587) |
| ~~`D2c` (the finding)~~ | | | ⛔ **A SHIPPED DEFECT, FOUND BY `D2a`'s RE-MEASUREMENT AND PRE-EXISTING** — [`probe/d2` § 10](../../probe/d2/README.md). Near wall **12 edges**, left **9**, right **4 of ~9**, far **2 — its corner posts and nothing between**. ⚠ Controlled at `3e3ac22`: identical, so `D2a` did not cause it. ⚠ Asked at **twelve heights** before it was called missing — *absent* and *stamped a storey up* read the same at one `ref` and want opposite fixes. ⚠ **It is invisible because the mesher draws from the RUNS**, which are all four and filed correctly, so the picture has been right and the store wrong the whole time — the same class as `D1a.1`, where the store was byte-identical and the drawing changed. ✅ **AND THE CAUSE IS LOCATED — `probe/d2/stamp.loft`, § 11: IT IS THE ROOF, AND THE STAMPING IS INNOCENT.** All four sides stamp completely and every write survives — 22 + 20 + 22 + 20 = **84**, which is exactly the number `place_house` prints, so the sentence is honest and what it describes stops being true four lines later. Counted at each stage: **floor 0 → walls 84 → roof 46**, with the far run going **24 → 4** and the near run **24 → 24 in the same measurement**, one line apart. `roof_over` calls `world_merge_band_as(w, q, r, floor_h, hu, …)` per footprint cell — a band from **the floor** to the ridge — so it rewrites the whole wall height of every column inside the house and takes the edges with it. ⚠ **The roof is not writing a roof; it is rewriting the column from the ground up.** ⛔ **The ASYMMETRY is still unexplained and two accounts are already refuted** — *a row offset* (the footprint is `r −1..3`, each wall has exactly one row inside it: symmetric) and *an edge dies when either cell is in the footprint* (fits the far run exactly, fails on the near run, whose edges also have a footprint cell and all survive). ⏭ What fits all four is **edge ownership by direction**, and it is **named as a guess, not measured**; the decisive experiment is one cell wide and written down in § 11 |
| ✅ **`D2c.1`** — `floor_h` leaves `roof_over`'s signature | XS | ⚠ **the suites are not the check** — a dead parameter compiles either way, so what says it is dead is that the BODY holds one occurrence and it is the signature | **DONE 2026-08-18.** `D2c` moved the band's start from the floor to the eave, which killed this parameter's last reader: `rp.rp_eave` is the only height the function needs and the plan already carries it. ⚠ **A parameter every caller passes and nothing reads is worse than dead code** — it tells the next reader the roof is seated on that number when it is not, and `place_house` was handing it `seat`. ⚠ **Removed rather than REFUSED, which is the opposite call from `hip_steps` one line below, and the difference is whether there is a fact to check**: `hip_steps` names a capability this function does not have, so it is refused out loud and a caller asking for one is told; `floor_h` named a height that is now the plan's, so there is nothing to validate and the signature should stop asking. Eight call sites — six in `tests/roof.loft`, `place_house`, and `probe/d2/stamp.loft` · roof **10** and house_walls **7** green, ⚠ **run SEPARATELY because `loft test` takes ONE target per run** — handed two it reported `1 file`, which is `D0`'s trap met a second time |
| ◐ **`D2b`** — `place` asks *is there a building here*; **`storey` does not, and the corpus is why** | S | ⚠ the meadow must refuse and the corpus's own storeys must still build — a gate that only ever says no is `D2p`'s row C with the sign flipped | **HALF DONE 2026-08-18, and the other half is REFUTED BY ITS OWN VERIFY.** ⛔ **THE TWO CLAUSES ABOVE ARE THE SAME CASE, WHICH THE CORPUS SAID AND NO READING HAD**: measured, **all three live storeys — `deck`, `cellar` and `threshold` — run over bare raised ground with `verb place` count ZERO in every one of them.** So *the meadow must refuse* and *the corpus's storeys must still build* cannot both hold, and a building gate on `storey` refuses every use the editor actually has. ⏭ **The storey half is dropped and recorded rather than built** — what the measurement says is that **a storey is a TERRAIN gesture in this editor, not a building one**, and `D2p` read its own control row as a defect list when its job was to prove the probe could reach a second verdict. ✅ **The `place` half is built and is exact**: a second house on the same footprint is refused, and **it writes nothing** — the shape `D2p`'s row C2b found missing next door, where a refusal had already stamped 55 cells and filed 8 runs. ⚠ **The gate asks about the FOOTPRINT, not the author** — `pose_footprint` puts the house AHEAD of the author, so *am I standing in a building* would refuse a person in their own doorway placing onto open ground; the question is whether the NEW footprint overlaps a filed plan. ⚠ **And the predicate is the MESHER's** — `roof_plan_covers`, the same one `hex_mesh` draws from — so it refuses exactly the cells that would end up under two roofs rather than re-deriving *inside* a second time. ⚠ **The control that stops it being *refuse everything* is a live script**: `seat.keys` places two houses **46 units apart**, so that distance is the row, and it was green before the gate existed as well as after · `tests/house_walls.loft` 6 → 7, the new row seen red first |
| ⛔ **`D2`** — verbs consult the mode: a verb with no binding here **says why** | S | a verb silently doing nothing. ⚠ *reason, offer, residual, never a blank no* | ⛔ **THE BINDING TABLE AS DESIGNED IS REFUTED AND THE ROW IS SPLIT** — `D2p`'s measurements, above. What survives is not *the mode grants the building verbs*: `opening`'s own precondition is exact, stable and already spoken, and the mode is unstable at the wall and structurally absent for 16 of its 18 real uses. What is left of the row is `D2a` (the line) and `D2b` (the gate the two ungated verbs actually need), and the doorstep sentence belongs to whichever of those refuses |
...
| ✅ **`T1d`** — the wire carries a VERB BY NAME, so a declared one can travel | S | ⚠ **not the word going over** — the two drivers' worlds byte for byte on a script that uses a verb no compiler saw, plus the typo floor that must still fail the run | **DONE 2026-08-20.** ⛔ **`T1a.2` SHIPPED A TYPE'S OWN VERBS AND THEY COULD NOT TRAVEL.** [EDITING_MODES](../../doc/claude/EDITING_MODES.md) says *the wire carries the verb, never the key*, and this socket carried neither: a message id per gesture, with `tools/script.mjs` holding a constant verb→id table. Enough for verbs a compiler saw — and a declared verb is a word no compiler ever will see, so `verb portcullis` worked headless (`editor_run` hands the word to `press_verb`) and answered *no verb 'portcullis'* over the wire. `55:<verb>` is `press_verb`, alias resolution and all — **the same call the runner makes, so the two drivers are one body**. ⚠ **IT DOES NOT REPLACE THE DEDICATED IDS, and that is a decision rather than caution**: each of those handlers owns a SENTENCE `K3` deliberately left in the server when it moved the gestures into the library (`fenced N edges, M stored outside`), so routing every verb here would replace all of them with one generic line and redden the gates that read them. The table is consulted first; `55:` carries what nothing else can. ⛔ **AND REMOVING THE TABLE'S REFUSAL SILENTLY DROPPED `K3a`'s FLOOR** — *a word the driver cannot send fails the run* used to fire on the table having no row, and with every word now reaching the wire a typo would have sailed through at exit 0, which is the exact silent no-op `K3a`/`K3b.1` were bought to end. The floor moved to the party that can now answer: the handler's `no gesture for X` reply. **Measured — `verb plce` exits 101; `verb portcullis` is performed** · ✅ `probe/s2c` **10 scripts byte-identical**, and its new `htverb` row catches a server that drops the message (served opened **0** edges against the runner's **1**) · ⚠ **and that sabotage corrected the probe's own wording**: its DIVERGES line named *the server's `36:` handler is a second body*, a cause it cannot know, which is `D4`'s *"the camera moved"* one probe over — it states the fact now · `probe/k3d` 32 → 33 · `make fast` rc 0 |
| ✅ **`T1e`** — `verbs_here()` returns, with two consumers: the BAR and the KEYBOARD | S | ⚠ **not the query** — the bar must offer the declared verb on its declared key, the verb it displaced must still be listed keyless, and pressing that key must reach the gesture | **DONE 2026-08-20, and it finishes `T1`.** A declaration says `portcullis:opening:F` now — the KEY was the blocker, not the world (`Client` has held `st.cache` since `B1b.1b`). `verbs_here` is the person's map plus whatever the type standing here declares, with the same three refusals `verb_resolve` makes, **asked again here** because a bar offering what the dispatch refuses is an editor advertising a gesture it will not perform. ⚠ **A DECLARED KEY REPLACES, IT DOES NOT REFUSE** — `keymap_bind`'s own rule: all 26 letters are taken (counted at `K3`), so a type that could only have an unused key could never have one. The displaced verb keeps its row with **no key**, which is the truth about it. ⚠ **ONE DERIVATION, TWO READERS** — `local_verbs` per frame, before the bar rebuilds; the bar draws it and `verb_down` looks every key up in it. ⛔ **AND THAT LAST WORD IS THE FIX THAT MADE IT WORK**: `verb_down` asked `st.keys` (the PERSON's map), so a declared verb had no code and did nothing **while the displaced `fence` still fired** — two tables, both wrong, and one map fixed both. ⛔ **THREE OF MY OWN INSTRUMENTS WERE WRONG BEFORE THE SUBJECT WAS**: the derivation sat inside `poll_input`, so the bar read a map built one call LATER and showed 15 base slots with the query working perfectly; the probe then read `grep -m1` — **the boot bar, before any rebuild** — and called it a failure with both lines in the transcript; and the keyboard row asserted an opening was CUT, going red on `portcullis refused — no wall here to open`, which is the gesture answering **having been reached through the alias**. It asserts DISPATCH now, and watches `no gesture for`. ⚠ **The page still cannot declare** — `T1c` gave `editor_run` a line and the server `54:`, local mode neither — so the proof runs under `AUTH_FIXTURE=castle`, a **fixture and not a sabotage**, declared separately so a green fixture run is never read as a passing sabotage; it also prints that B8/B10/B11 cannot hold under it, because the page's world is deliberately not the runner's · clean `probe-auth` **37 PASS, unchanged** · `make lib-test L=hex_editor` 618 both backends · `make fast` rc 0 · `make page-check` PASS |
...
| ✅ **`E1`** — the camera's ease is `fixstep::approach`, not ours | XS | ⚠ **not *the camera still works*** — an asymptotic ease has the same target, so a settled camera is identical either way and a green gate would prove nothing. What is asserted is the **composition property the retired form lacks**, plus the priced gap at the server's own four `k` | **DONE 2026-08-18.** dryopea's plan 26 `L6` extracted `fixstep 0.1.0` into `loft-libs-game`, and **this repo is named in it three times**: the package is not called `tick` partly because `hex_editor/src/tick.loft` already is one, `moros_map` was floated as a second tenant and refused, and **`fixstep/src/ease.loft` quotes our `cam_approach` verbatim as *"game B — same name, same signature, different arithmetic"***. Its own status line carried moros as the open half: *"that repo still holds the linear `cam_approach` and consumes nothing."* ⛔ **AND THE LIVE-BUG CLAIM IS FALSE HERE, MEASURED BEFORE IT WAS REPEATED** — `dt` is `hex_editor::tick_dt()`, a **constant**, and the pump works a backlog one fixed step per pass, so both forms were deterministic in this tree; and **no ANGLE is eased** (`cam_yaw` never passes through it), so `fixstep`'s wrap-around case does not apply either. What the swap buys is that frame-rate independence becomes a property of the **ease** rather than an accident of the **driver** — which is what `C3` needs the day the page takes the camera — and one home for a fact two games had written. ✅ **`probe/e1/ease.loft` holds the retired arithmetic and nothing else does**, so the comparison is against the real thing: one second cut two ways at k 6 gives the retired form **1.0** against **0.9982029897**, spread **0.0018**, and `fixstep::approach` **0.9975212478** both ways, spread **exactly 0** — with a guard on each direction that would say so if the probe stopped measuring it. ⚠ **The cost is priced rather than discovered**: a settled camera is identical, and the gap at the four `k` is 0.013–0.028 after 10 ticks, 5e-6 … **1.4e-3** after 60, and ≤3e-6 by 120 — `CAM_K_OUT`, the slow return, carries it longest. ⛔ **What is NOT verified is the picture**, for `D2a.2`'s reason: `camera_indoors` is one of the five gates down on loft#950 · `make headless-same` rc 0 · the three camera scripts `probe/k3c` row B controls still exit 0. ✅ **AND THE DEPENDENCY WAS CHECKED ON THE TARGET WE SHIP, WHICH ITS OWN AUTHOR COULD NOT**: `fixstep`'s README marked `--html` **`n/a` on an argument** — *"pure loft, no `#native` bindings, so no bridge is involved"* — which is sound reasoning and not evidence, in the week loft#950 proved an `--html` build can diverge with no bridge at all; and `ease_fraction` is `1 − exp(−k·dt)`, so it rests on a builtin. `probe/e1/ease_html.loft` runs the composition identity, the four `k` after 120 ticks, `ease_fraction` including the paused frame and the angle half, on the interpreter and then as a 386 KB page driven off `file://`: **all 11 lines identical**, `compose … spread 0` in a browser to the bit. Their table says *measured* now, with loft#964 on the `--native-wasm` row |
| — | | ⏭ **THE CLIENT IS TESTABLE HERE. Nothing below starts before this line.** | |
| ✅ **`C1`** — the sampler probe: `surface_h_at` as a `fn(…)` parameter, camera pixel-identical | XS | ⚠ **the row said `subject 0.0188` and that number is STALE** — the live first frame is `0.0206`, drifted before today; verified against a baseline measured an hour earlier, over **nine** frames rather than one | **DONE 2026-08-18.** `cam_clear_at` takes the surface as `sample: fn(integer, integer) -> float`; `eye` and `wld` are both out of its signature, so the routine no longer knows how anything stores a surface — which is the single thing standing between it and `hex_cam`. ⚠ **`eye` went with it rather than staying for tidiness**: it was only ever passed through to the sampler, and a parameter the function does not use is one the library version would have to explain. The closure captures it where the eye's height is already known. ⚠ **Smaller than the row implied** — `surface_h_at` had exactly ONE consumer, `cam_clear_at`, so this is two call sites and not a sweep. ⚠ **And it is not a bet**: `cliff_edges` and `ground_axle` already take terrain by a function, so the mechanism was proven in this tree before it was used here. ✅ **Verified pixel-identical over nine frames** across FOLLOW, SNUG, CUTAWAY and EYES — `0.0206 0.0595 0.0416 0.0595 0.0003 0.0594 0.0088 0.0594 0.0866`, byte-for-byte before and after. ⛔ **THREE COMPARISONS BEFORE THAT ONE WERE VACUOUS AND EACH READ AS A VERDICT**: the baseline pattern missed the gate log's backslash-escaped JSON (0 frames), a passing gate prints 135 bytes and carries no rows at all, and `tools/script.mjs` standalone exits 13 because it needs a SERVER to attach to. **An empty set compares unequal to everything**, so each produced a confident `MOVED`. The check counts its frames now and says *this says nothing* at zero. ⚠ **And the stronger instrument was lost by an ordering mistake**: the twelve `shots/*.png` would have been a literal byte-identity check, but they are untracked and the running gate overwrote them before they were saved. Snapshot them BEFORE the first run |
| ✅ **`C2a`** — `lib/hex_cam/` holds a COPY of the routines, with its OWN tests | S | ⚠ passes the lower bound because those tests are real geometry that can be surprised — unlike a declaration checked against itself | **DONE 2026-08-18.** `lib/hex_cam` — **zero dependencies**, 8 tests green on both backends, in `LIB_PACKAGES` so it cannot rot unnoticed. It holds the clearance fit, the boom and the pivot; the server still runs its originals until `C2b`, which is the plan's own shape — the copy must be green on its OWN tests first, so a later red is attributable to the MOVE rather than to the routines. ⛔ **`C1` MEASURED THE REAL CONTRACT AND IT IS THREE SAMPLERS, NOT ONE.** `cam_free_dist` asks the world three distinct questions that do not collapse — interpolated ground at a continuous POINT, the SURFACE at a cell, and the TERRAIN at a cell — so the marching routines stay behind until each is a parameter. A library taking one sampler would be quietly wrong at two of them. ⚠ **THE LATTICE IS THE CONSUMER'S TOO**: `cam_clear_at` takes `nb` and `px` as functions, so this package carries no hex geometry and crawler's lattice can differ. ⚠ **And every length is the FIGURE's** — `cam_boom(fig)` rather than a constant — because crawler's figure and the editor's are not the same height, and a boom fixed in world units frames one of them wrongly. ✅ **THE TESTS CAUGHT THE AUTHOR'S ARITHMETIC, WHICH IS WHAT SAYS THEY CAN BE SURPRISED.** The slope row first asserted `0.6364` — the answer for INDEPENDENT axes — and went red at `0.675`: the fixture's diagonal neighbours couple x and z, so a pure-x slope still yields `gz = 0.5`. The function was right. The derivation is written out in the test rather than the observed number pasted in, because a row that re-derived this from the subject's own expression could not have caught it |
| ✅ **`C2b`** — the server calls the library copy; the private originals are deleted | S | ⚠ the nine frames unmoved — the row said `subject 0.0188` and that number was stale, see `C1` | **DONE 2026-08-18.** The server calls `hex_cam` for the clearance fit, the boom, both pivots and the eye height, and **its own copies are gone**. What is left is the ADAPTER, and the adapter is the seam: the library asks for a cell plus two lattice functions, and this file is where moros says which lattice it uses and how it stores a surface. `C1` handed over the surface, `C2b` hands over the lattice — so no hex geometry and no store reaches the package crawler will share. ⚠ **THE ORIGINALS ARE DELETED RATHER THAN LEFT BESIDE IT** — two bodies of one derivation is `D2a.1` and `T1a` over again, and a private copy that still compiles is the one a later edit lands in. ⛔ **AND THE SHADOWING HAZARD WAS WALKED INTO IN THE SAME EDIT THAT DOCUMENTED IT.** `const CAM_CLEAR` was deleted here on the grounds that a program declaring a name a library publishes shadows it invisibly (`HOUSE_W`, measured) — and four wrapper functions were then left carrying the library's OWN names. ✅ **loft refused it by name**: *Cannot redefine 'cam_boom' (already defined at lib/hex_cam/src/hex_cam.loft:91)*, both sites in one line, which is the fix CLAUDE.md records landing where it counts. The adapters are `*_here` now, matching `cam_clear_here`. ⚠ **And the vacuity guard earned its place**: the failed compile produced ZERO frames, and the comparison said *this says nothing* instead of a verdict — an empty set compares unequal to everything, so it would otherwise have reported a confident `MOVED` from a run that never happened. ✅ Verified: nine frames identical across FOLLOW, SNUG, CUTAWAY and EYES — `0.0206 0.0595 0.0416 0.0595 0.0003 0.0594 0.0088 0.0594 0.0866` |
| ✅ **`C3`** — the page client takes the camera | S | ⚠ **the demo, and WHICH rows moved** — a camera change that shifted a world key would be reaching something it has no business touching | **DONE 2026-08-18.** The page's last three camera numbers are gone: `LOCAL_BOOM 5.0`, `LOCAL_LIFT 3.2`, `LOCAL_AIM 0.9` — a THIRD set beside the server's and the library's, fixed in world units and derived from nothing. ⚠ **The duplication was not the defect; the fixed world units were** — a camera framing a person is sized by that person, so `hex_cam` keeps the ratios and takes the figure, and a boom of 5.0 was right for one height and silently wrong for any other. ✅ **Two behaviours the page did not have**: the pivot LEADS the character (it aimed at their own spot, which is staring at a back rather than following), and the eye is lifted by the clearance the surface asks for — the page's flat 3.2 let it graze a hillside. ⛔ **AND THE ONE FAILURE WAS NOT THE CAMERA'S.** `probe-demo` came back with exactly one red — `F4 the first house built world 41145:635835879, not 41145:1306471549` — and that literal was recorded at `B1c.1` on **2026-08-13**, before `D2a` raised the ridge and `D2c` gave the house back two walls. The probe could not have said so earlier: **loft#950 had it unrunnable for the two days those fixes landed**, which is `D2a.2`'s *44 gates that could not see it* one probe over. Same 27 cells, different content. ⚠ **RE-BLESSED UNEVENLY, AND THAT IS THE POINT.** `F4` took the new literal; **`Q2` and `G2` stopped being literals at all** and compare `$f_key`, the value measured in the same run — which the script's own comment already CLAIMED they did. That gap is why one moved claim failed three rows. Pasting the key into all three would have gone green while leaving two rows that break on the next geometry change, and `T1`'s house types change house size by declaration · `make probe-demo` rc 0, zero failures |
| ◐ **`C4`** — hand `hex_cam` to crawler | S | ⚠ **needs a word first** — and the word came back *do not change crawler* | **THE ROW CANNOT BE DONE AS WRITTEN, AND THAT IS THE ANSWER RATHER THAN A BLOCKER — 2026-08-18.** *Hand* implied an edit in crawler's tree, and `../crawler` is **read-only**: another agent works there, and [CLAUDE.md](../../CLAUDE.md)'s reason is not politeness — *an edit it did not make destroys its ability to tell its own work from yours*. The project owner confirmed it on 2026-08-18. ✅ **Verified rather than asserted**: crawler's working tree is untouched and its last commit is 2026-08-12, six days before this work. ✅ **SO THE DELIVERABLE IS THE NOTE BELOW, AND THE PACKAGE STAYS HERE.** `lib/hex_cam` exists, is published-shaped (zero dependencies, its own tests, in `LIB_PACKAGES`), and crawler consumes it if and when its own agent chooses. Raising a finding is what that tree takes instead of a commit. ⚠ **AND THE NOTE HAS TO SAY WHAT IS *NOT* IN IT, or it is an advertisement.** `hex_cam` holds the FRAMING — `cam_clear_at`, `cam_boom`, `eye_height`, `cam_pivot_x/z/y` and the four ratios. It does **not** hold the boom march: `cam_free_dist`, `cam_free_side`, `cam_free_arc` and `cam_pitch_target` are still `src/editor_server.loft`'s, because `C1` measured that they ask the world **three distinct questions that do not collapse** — interpolated ground at a POINT, the SURFACE at a cell, the TERRAIN at a cell. **So a consumer wanting OCCLUSION rather than framing cannot have it today**, and that is a further slice rather than something `C4` could hand over. ⚠ **What crawler would supply**: its own lattice (`nb`, `px` are functions), its own surface sampler, and its own figure height — every length here is a RATIO, which is the whole reason the package can be shared by a game whose figure is not 1.75 m |
