...
**Next:** **`D2b`** — the *is there a building here* gate the two genuinely ungated verbs need.
**`T1`** is independent of it. ⏭ And a loose end `D2c` creates: **`floor_h` is now unread in
`roof_over`'s body**, since `rp` already carries the eave — a dead parameter across eight call
sites, to remove as its own validated step rather than bundled.
