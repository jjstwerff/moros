...
| **`D2b`** — `place` and `storey` ask *is there a building here* | S | ⚠ the meadow must refuse and the corpus's own two storeys must still build — a gate that only ever says no is `D2p`'s row C with the sign flipped | ⏭ **The two verbs `D2p` found with no precondition of their own.** A floor on 19 cells of open grass and a second house inside the first are both accepted today, and the registry that can answer them exactly is `es_roofs` — which is a question about a BUILDING rather than about the author's shelter, so it is not the mode and must not be spelled as one |
