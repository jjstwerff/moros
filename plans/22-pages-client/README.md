...
| **`K2b`**–**`K3`** — convert the rest, then drop the key spelling | M | a converted script and its original build the same world **and the same session** | ⏭ **Two keys left** — `B` and `C` (`R`, `Y`, `T`, `J`, `K`, `V`, `E`, `Z`, `X` done; `Q` bound and pressed by no script). ⚠ **And they are the first pair whose scripts have a REAL GATE**: `deck.keys` and `cellar.keys` are driven by `deck_soffit.mjs` and `cellar_ceiling.mjs`, where the other eight are run by hand |
