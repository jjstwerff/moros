...
| **`D2a`** — the mode's line includes the wall an author stands on | S | ⚠ **not *the flip moved*** — it must move PAST the walled band and leave the meadow alone, or the fix is *everything is inside* | ⏭ **UNBLOCKED, and it is the half of `D2` the measurements support.** `place_house` files a roof plan whose rectangle is the wall's own line and the shelter query tests it strictly, so the person on the wall is out. Measured candidate: **one cell each way**, flip `−2.00 → −3.50`. ⚠ **It is not the mode's to fix alone** — `shelter_at` is the CAMERA's query too, and this design's own rule is *do not add a second enclosure test*, so the eave belongs in the plan and both readers move together. That consequence is the step's to measure, not to assume |
