...
| the walk | `W S A D` + the mouse, shaped like a CONTROLLER — [CONTROLS](CONTROLS.md) §8, 2026-09-08: the walk is a VECTOR (`hex_editor::move_of`), `A`/`D` **strafe** and the mouse turns the character, on the page too; a real gamepad drives the same three numbers through the page's queue and `61:` on the wire. ⚠ The first page probe read every drag as HALF — the walker line's `turned` counted yaw only on ticked frames, and the camera lagged the mouse for the same reason. `probe/stick` |
| the plan | `m` (on the page) — plan 26 `B6`: a plan of where you stand, over the canvas, drawn by the same `plan_levels` as `make plan-view`; a click on it is a `pick` and the next verb lands there with the walker unmoved; `m` again takes it down. `probe/b6` reads every claim out of the DOM. ⚠ `raise` from a pick lands `PEAK_AHEAD` hexes along the facing, not on the clicked cell — the plan is a MODE and its ground verbs want their own binding, not built |
| deformation | `push` (`2`) — take the cell you face into the set you are standing on: a road extends, a room floor grows, a tunnel lengthens, **one gesture with no selection**. ✅ **And the walls follow now** — law `L1` of [WALL_PUSH](WALL_PUSH.md): `∂` is a function, so a push does not MOVE a wall, it changes one cell's membership and only that cell's six edges can have changed status. The id is CARRIED, so a room walled in its own type stays that type and a boundary already standing is never repainted; where there is no wall in front of the author nothing is written at all, which is why a road stays byte-identical. ✅ **And it is HELD now, over a walk** — `G2`: `press_verb` flips the mode, `walk_tick` performs one transfer per tick, and all three drivers carry one boolean across, so `2` means the same thing in the server, on the page and in the runner. ⛔ **Every tick and not once per hex — and the first argument for that was REFUTED by its own sweep.** It said the per-hex trigger would deadlock; row 2 of `probe/wp/sweep-g2.sh` IS that trigger and went **green**, because a walker meeting a wall is standing in a hex they have just entered. ✅ What separates the two is **turning**: a push is aimed by the FACING, and a facing changes with no hex entered at all. What makes per-tick safe is `push_cell`'s `already` — a thousand ticks standing still take the one cell one tick takes, `w_tau` unmoved. ⛔ **It still does not pay `L11`, and the reason is the CONTROLS**: `W` walks along the facing and the push takes the cell along the facing, so **you can only push where you walk** — a walk drives a one-cell **corridor**, and `house_recover` reads a house before it and refuses it after. Sweeping a face needs walking ALONG it while facing it, and there is no strafe. ⛔ Its §7 is the rest of what is NOT built: an internal partition is **not reachable at all** because membership is the MATERIAL, and the filed `WallRun` records are left where they were — which is [EDITOR_DEFECTS](EDITOR_DEFECTS.md) 4's *every wall is drawn twice* coming apart |
...
| `src/plan_view.loft` | a **plan of a saved world**, as SVG. No clock at all — plan 26 `B0`, `make plan-view`. It draws the field and the DESCRIPTION recovered from it: a straight run, a disc, an octagon, or a HOUSE's rectangle — and which of the two round readers is asked is the **palette's** decision, not the field's (`B4m`). ⚠ The house's rectangle comes from its FLOOR, not its marks, and the caption names the marks it does not explain: **4 per house, one per corner** — `B0`'s oldest open question, answered at `B4n`. ⚠ **A window holds more than one description now** (`B4o`): a mark is attributed to a structure before anything is asked what it describes, which is what closed `B1`'s *a house eight hexes away makes an unrelated wall unreadable*. ⚠ **And the leftover gets the WHOLE chain** (`B4p`) — a house and a round or octagonal tower are both drawn, with the LEFTOVER's own palette picking which round reader is asked; and (`B4s`) a window describes **every** structure the readers can isolate — a house, a tower and a wall draw three descriptions with nothing unexplained, because each one's own marks are subtracted before the next question is asked. ✅ **And two of a kind too** (`B4t`): the field is split into components before any reader is asked, so each is handed a field that is entirely one structure's and the both-ways comparison — what stops a disc being accepted with a wall across it — is left alone. Five structures, five descriptions. ✅ **And structures that TOUCH are drawn apart now** (`B4u`): the MARKS are flooded through the vertices they share while the floor keeps the cell flood, so two walls two cells apart, a wall running off a house and a tower a cell from one are each their own structure — where `B4t` refused them, or drew a **`run` over the tower's rim**, which is a wall nobody built. ✅ **And a component no ONE reader explains gets a SET of them** (`B4v`): each candidate's boundary must lie WITHIN the component's marks, and the set is taken only when it claims every one of them and nothing outside — `FORMAL_CORE` §6's R1 over the union rather than over one shape, in one integer measured both ways. A wall MEETING a rim draws both, and two rims that meet draw two circles, where `B4u` counted 52 and 106. ✅ **And a CROSSING is two runs** (`B4w`): a wall can only begin where the chain does — a free END or a JUNCTION — so the candidate pool is a handful of exact corners rather than a search over the plane. ⚠ **A `T` is a measurement, not a category**: over eight stem positions along one crossbar, TWO close and six are refused with every mark counted, because a stem ends at a continuous point on the crossbar's centreline and is recoverable exactly when the field's own corners carry it. That is `FORMAL_CORE` §6's R1/R2 line in the field, and neither outcome is wrong — the union test is the acceptance either way. ⚠ And the first picture of it found `B4o`'s bound was exact only at the distance it was measured at — a house captioned `11 stray` where its mitre is four, because a neighbour's rim reaches inside `house_owns`; the residual is confined to the claim set now. ⚠ **And a run's description is measured against the field it came from** (`B4q`) — `· N stray · N missing`, the same grammar the house uses — which is how [EDITOR_DEFECTS 7](EDITOR_DEFECTS.md) was found: **the same wall walked the other way is a different field**, because `wall_stamp` took its halfplane normal from the run's tangent — ✅ **fixed at `B4r`**, where the sign was only half of it and the offset had to be anchored at the MIDPOINT too. ⛔ **AND A WALL THAT TURNS IS THE NEXT GAP, MEASURED TWICE AND NOT BUILT** (`B4x`): a corner where two runs meet is neither an END nor a JUNCTION, so the pool is empty there — a **zigzag** of three walls draws two runs and loses **12 of its 18 marks**, and a **closed room** of four draws **nothing at all** and loses all **50**. Admitting every corner closes the zigzag with both controls unmoved, and blows `loft test`'s 300-second deadline on the room. ✅ **THE ORDERED CHAIN WALK `CLAUDE.md` NAMED AS THE MISSING ENTRY POINT IS BUILT — `hex_shape::wall_chain_walk`**, beside the two counters that answered *how many* and threw the walk away, with five contract tests and a six-row sabotage sweep whose two controls stayed green ([`probe/b4x`](../../probe/b4x/README.md), `make probe-b4x`, which is its consumer check). ✅ **PUBLISHED as `hex_shape` 0.1.2** — tag `hex_shape-v0.1.2`, release asset sha `48178c8f…`, [registry#26](https://github.com/loft-lang/registry/pull/26) merged and **signed** (trust gate verified under a client-trusted key), so `make probe-b4x` now resolves the walk from the registry like any other consumer and `lib/hex_editor/loft.lock` moves to 0.1.2. ✅ **The ACCEPTANCE is settled and the first measurement's prescription is refuted**: exact set equality does *not* fail on a straight wall — rebuilt, it closes every fixture in one piece — and **within-ness on its own is UNSOUND**, clean at only **12 of the room's 50 seeds** and losing up to **45 of 50 marks**, because the number it maximises is the run's own size and not how much of the chain it accounts for. Both **two-way** tests (`FORMAL_CORE` §6's R1 in both directions, `B4v`'s sentence one level down) are clean at **50 of 50**. ⛔ **The PARTITION is NOT settled**: greedy gives **4…9** pieces for a four-wall room and the minimum-cost shortest path gives **4…7** at eight times the cost, so *the fewest runs* was worth its 1012 `run_edges` only if the answer stopped moving, and it does not — a linear cut of a **cycle** from a scan-chosen vertex is not that cycle's minimum. ⚠ Its corner control is what makes that a diagnosis: the room's four walls measured *alone* are one run each at 12/13/12/13 — **exactly its 50** — so the right answer is available at every seed and every rule tried misses it at most of them. ⛔ **And the whole first write-up was one seed reported five times**, which is this tree's own instrument rule one level in. ⛔ **AND THE WHOLE ROW ABOVE WENT STALE FOUR HOURS AFTER IT WAS WRITTEN**: `hex_editor::run_chain_within` shipped in `13db614` (2026-08-28 20:39), wired into the peel and the plan view — **three code files, no document** — so this file, the plan, `FOCUS` and `CLAUDE.md` all went on saying *a closed room draws nothing* and a session was planned against it. ✅ **Measured 2026-08-29 through `hex_mesh::plan_svg`, the same room is four runs with nothing missing**, and the real number is **8 of 25**: a rectangle authored as four walls is described as four in 8 cases (the 7 closed chains plus `8 × 3`) and as **5 or 6** in the other 17 — exactly the ones carrying a junction — with no marks left undrawn anywhere. ⛔ **And the same room moved gives 4 4 4 4 6 6** over six half-cell positions, every LOOP position giving 4, so **the loss is the corner and not the cut**. ✅ **AND THE PARTITION ON A CYCLE IS SETTLED — the third measurement, 2026-08-29**, though it beats the shipped peel on no fixture measured. A cycle has no start, so do not choose one: `run_between` takes two coordinates, so **feasibility is a property of a vertex PAIR** and the table is rotation-invariant — the fifty-seed sweeps were paying for one table fifty times. Build it once, take the linear minimum from every start against it, and the room comes back as its **four walls, 13/12/13/12, uniquely, nothing over**, for **2454** `run_edges` against 1012 for a single seed. ⚠ **The instrument was checked against a number already measured live**: the table records WHICH HALF of the two-way test passed, so `dp_once`'s within-only merge clause replays off it for free and reproduces `dp_sweep`'s `4 5 5 4 6 7 4 6` value for value. ⛔ **AND THE CLOSED CHAIN IS THE LUCKY CASE — 7 of 25.** Over 25 closed rectangles only 7 are one closed chain and **17 carry a junction**; `8 × 3` is four separate chains. So *a closed room draws nothing* is the 28% case; what the shipped peel makes of the rest is not measured, only that their pool is non-empty. ⛔ **On a broken chain the minimum is NOT unique** — 5 optimal four-piece partitions for a 14×6 room, 16 for an L-shaped one, and they are four different pictures rather than a one-vertex wiggle: a chain starting mid-wall begins with a **fragment**, and a fragment can be spent anywhere. ✅ **AND THE CORNER IS MEASURED — the break is ONE HEX EDGE wide.** All 80 breaks over the 25 rectangles are **at a corner**, none mid-wall, and **76 of 80** have another break exactly one hex edge away; the steps are `(0,3) (3,0) (3,-3) (0,-3) (-3,0) (-3,3)` and nothing else, which is `hex_edge_corners`' own `d = 0…5`. ⚠ **So *one edge* is an integer, not a radius**, and `B1` is now a decision — join chain ends across a one-edge break in the reader, or leave the corner closed in `wall_stamp` — with the control first — and it is **built and green**: over 14 fixtures of two walls at gaps 0.2…2.6, collinear and parallel, **no unrelated pair has ends one hex edge apart** (nearest √3), while `8 × 3`'s four broken corners report four. ⚠ Its first version passed for the wrong reason (the walls were two edges apart), which is why the instrument is now checked against a fixture that HAS the thing |
...
⛔ **AND IT MOVED UNDER A RUNNING SESSION THREE TIMES — 2026-08-29.** `~/.local/bin/loft`
was `7f2bae7f…` at the start of the session, `2a6739b1…` in the middle and `a44c94d9…`
(09:40) by the end, with no action from this tree. Everything measured before the swap — an 813-test suite, a 58-test file, a
...
| [#1164](https://github.com/loft-lang/loft/issues/1164) | ⛔ **a parenthesised slice with an arithmetic index is refused** — `(s[i + 1..])` fails with a spurious *missing argument for parameter `v2` of `OpAddInt`*, while the identical slice outside the parentheses compiles | ⛔ open — **workaround: hoist the slice**, done at `editor_run.loft:627`. ⚠ **It cost a whole probe**: `src/editor_run.loft` would not compile, so all 18 scripts `probe-k3c` drives produced EMPTY logs and its 20 red rows were every one of them downstream of this line. Not a regression — both installs refuse it |
| [#1162](https://github.com/loft-lang/loft/issues/1162) | **25 test files in `moros_sim` / `moros_editor` / `moros_render` failed with `index out of bounds: the len is N but the index is 65535`** | ✅ **fixed by the next install** — red on `2a6739b1…`, green on both `a2c3faed…` before it and `a44c94d9…` after, **same source and identically resolved dependencies**. Closed. ⚠ The lesson outlives it: `2a6739b1…` was a short-lived intermediate that a whole tier of green was measured against |
