...
| #912 | a module's **FILENAME** is global across the dependency graph — a consumer's new `src/<x>.loft` shadows a dependency's, and the DEPENDENCY's own internal calls stop resolving | ⛔ **OPEN, filed 2026-08-14.** Cost: `hex_mesh/src/catalogue.loft` against `hex_part`'s, reported as *"Unknown function `part_list`"* against a `pub` function in a package green on its own — found by elimination, not from the diagnostic. Two-package repro; ⚠ **the filename ALONE is the trigger** (no `use` in the consumer, no shared declared name) and the control is renaming it. Now a rule in CLAUDE.md, which said *grep before adding a public NAME* and never mentioned basenames |
| #913 | `loft test` refuses the test path it PRINTS — `tests/x.loft` is joined to `tests/tests/x.loft` | ⛔ **OPEN, filed 2026-08-14.** `sev:low`, `wa:clean` — drop the prefix. Costs one round trip every time a red suite is narrowed to a file, which is exactly when nobody wants one |
