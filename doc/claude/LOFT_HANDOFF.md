...
| #891 | `host_input()` **blocks** with no host, so a program cannot ask its environment anything optional | ✅ **FIXED — re-measured 2026-08-17** on the toolchain installed 2026-08-16 23:08. `probe/b1b/ask.loft` answers `got [] len 0` and exits **rc 0** on the interpreter and on `--native`; it was `timeout 20` → rc **124**. ⚠ The instrument is the timeout itself, and it has been seen firing on this exact command, which is what makes the rc 0 a result rather than a silence |
| #912 | a module's **FILENAME** is global across the dependency graph — a consumer's new `src/<x>.loft` shadows a dependency's, and the DEPENDENCY's own internal calls stop resolving | ⚠ **CLOSED UPSTREAM 2026-08-16, AND HALF OF IT IS LEFT — re-measured 2026-08-17.** The fix is an `Advice[module-name-shadowed]` naming both files, the `use` site and which one binds. It **fires when the collision resolves and is silent when it breaks the build**, which is the case this was filed about: still `Unknown function part_list` at `dep/src/dep.loft`, with a spurious `missing argument … of OpAddInt` beside it and no advice. Filed as **[#948](https://github.com/loft-lang/loft/issues/948)**. The other variant is worse and is a design question: the consumer's file WINS, so the dependency's own `dep_answer()` returns **100** where its own module says 42, with only an advice — **[#949](https://github.com/loft-lang/loft/issues/949)** |
| #913 | `loft test` refuses the test path it PRINTS — `tests/x.loft` is joined to `tests/tests/x.loft` | ✅ **FIXED — re-measured 2026-08-17.** `loft test tests/con.loft` runs the file it names (`ok tests/con.loft`), and the bare-name form still works, which is the control |
| #948 | the `module-name-shadowed` advice is **suppressed exactly when the collision is fatal** | ⛔ **OPEN, filed 2026-08-17**, the residue of #912. Two variants of one repro differing only in what the shadowing file declares: one prints the pair of filenames, the other prints two errors against another package's source and nothing else. Guess at the mechanism, unverified: advice is collected after resolution succeeds |
| #949 | a dependency's own internal `use` binds to a **consumer's** file | ⛔ **OPEN, filed 2026-08-17**, `needs-design` rather than `bug` — it is what *a basename is global* means, and it means a published, tested package returns a different answer because something downstream added a file. Measured: `dep_answer` 100 against its own 42, one advice, no error, same on `--native` |
| #976 | a package's own `use <mod>;` loses to a **sibling's** file of the same name, and the CONSUMER's `use` line order decides which package loses | ⛔ **OPEN, filed 2026-08-18**, `enhancement` `needs-design` `wa:clean`. The third shape of #912 and the first with **no dependency relationship at all**: `moros_sim` and `hex_part` each held `src/skin.loft` with no name in common, both suites green forever because a package's own graph holds only itself. A consumer pulling both gets `unknown type 'PartBox'` — or `Unknown function skin_overlap` if the two `use` lines are swapped. ⚠ **A qualified name does not disambiguate**, because the module never loads. ✅ `use self::skin;` in both fixes it in both orders, which is why the label is `wa:clean` and the ask is a DEFAULT rather than a fix: should a bare `use` prefer the package's own file? ⚠ It had not bitten only because both modules were test-only — the first production caller would have been the failure, against a file its author had not touched |
| #1012 | `loft verify-self` exits **0** when it verified nothing | ⛔ **OPEN, filed 2026-08-19**, `bug` `sev:low` `wa:clean`. Both installs on this box answer *"not a release bundle — nothing to check against"* with rc 0, so a corruption detector cannot tell *intact* from *unexaminable*. `loft audit`'s graded exits are the precedent |
| #1027 | `registry_validate.sh` validates whatever is cached LOCALLY and reports `OK` | ⛔ **OPEN, filed 2026-08-20**, `bug` `sev:medium` `wa:clean`. Run straight after publishing `hex_way 0.1.1` it validated **0.1.0** and printed OK: step 1 (`loft install $PKG`) does not refresh the index so the new version is invisible, and step 2 then takes the highest version present under `~/.loft/registry/`. ⚠ The version IS printed and the verdict is a bare `OK` — the same distinction #1012 is about: *checked the thing you meant* vs *checked something*. `loft api --registry --refresh` then pinning is the workaround |
| #1045 | the registry index and its `.sig` are **not swapped atomically**, so a crossing reader fails hard on a signature that was never wrong | ⛔ **OPEN, filed 2026-08-20**, `bug` `sev:low` `wa:clean`. A `make fast` that crossed a registry refresh died with *`registry index signature INVALID — refusing to install; this is a hard failure even with `--allow-unsigned``*; the identical re-run passed with nothing changed, and `index.json` + `index.json.sig` had both been rewritten mid-run. ⚠ **The message reads as a security event** — *doesn't verify against any known key* is what a tampered index produces, so the natural first move is to go through trust roots, a long way from concurrency. ⚠ **AND IT IS DEEPER THAN IT WAS FILED**: the loft side measured `std::fs::write` truncating before it fills, so a crossing reader gets a SHORT index in **194 of 200** concurrent reads — the invalid signature was the loudest of the failures available, with a JSON parse error equally likely. ⚠ **Fixed on an unmerged branch, NOT on `main`**, so **re-running is still the workaround** — and re-running rather than hunting trust roots is the correct read of a wrong message. ⚠ Several agents share one `~/.loft/registry` on this box, so a refresh landing inside a 16-wide suite is ordinary rather than exotic |
| #950 | the **`--html` build traps `RuntimeError: unreachable`** building part thumbnails | ⛔ **OPEN, filed 2026-08-17**, `sev:high` `wa:none` — a **regression** on the binary installed 2026-08-16 23:08, with the version string unmoved. The control pair is one page's own two prints: the previous binary's engine goes `local library — 20 parts` → `local thumbnails — 49 meshes for 20 parts` → 300 frames; the current one dies between them. ⚠ **Ruled out by measurement, not argument**: our own edit (unmodified source traps identically), the wire (same engine with no parts boots, raises, saves), and the library half — `hex_mesh::part_thumb_wire` answers **69 messages over 20 parts on the interpreter, `--native`, `--native-wasm` AND in an `--html` page with the same baked filesystem**, and `gl_upload_vertices` is clean in a page at 1…4096 vertices. Full record and the reduced programs: [probe/t5/README.md](../../probe/t5/README.md) |
