...
⚠ **AND IT IS NOT EVERY MESSAGE, WHICH THE INDEX SAYS IT IS.** Counted 2026-08-14 against
`src/editor_server.loft`'s own `MSG_*` constants: **thirteen ids have no row here** — `31`–`39`,
`41`, `42`, `50` and (until `K3` added them) `52`/`53`. That is not a small tail: `36:` is the whole
opening family, `37:` the annexes, `38:` the seats, `39:` the slabs. ⏭ **`39` has a row now** (`K3`
· `Z`, 2026-08-14) and it is the argument for filling in the rest: writing it down is what surfaced
that a bare `39:` means *slab* and therefore cannot carry the *empty means the one I chose*
contract the three ids beside it share. **Ten left** — `36` was written at `S2c`, `54` was born with a row at `T1c` (both 2026-08-18) and `55` at `T1d`, for the reason this note gives: its absence is what let the server keep a second body of the gesture with nothing pointing at it. They arrived after the
reverse-engineering above and nothing made them come here. ⏭ Filling them in is a real task and
this note is deliberately not it — what the note prevents is the next reader treating an ABSENT
row as *there is no such message*, which is this tree's commonest instrument failure wearing a
document's clothes.
...
| `30` | STAIR | `<+1\|-1>` | `stair ±1 at q,r height H from S` · `stair refused (C) why` · `stair refused — no cell along the facing` · `stair refused — a tread of nothing` | **A** — added 2026-07-31 → `stair_height` + `surface_set`. ⚠ **TWO VERBS AND NOT A FAMILY** (plan 22 `K3`, 2026-08-14): `E` names `stair_up` and `Q` names `stair_down`, so this message keeps its **signed payload** where `36:`, `37:` and `38:` went bare with a selection behind them. The rule is `raise`/`lower`'s — a DIRECTION is part of the action and a THING is not — so there is no `SELECT_STAIR` id and there will not be one. ⚠ **The whole gesture is `hex_editor::stair_ahead`**; what stays in the server is the `S:` and the dirty mark. ⚠ **And moving it FIXED two numbers**: the walk reference was `py / HEIGHT_SCALE` and the stride `cliff_step()` = `walker_step(W_UNIT)` — a global and a program constant, both 0.25 on the landscape and both wrong on a **part world**, where a tread came out half the height a walker can climb |
| `36` | OPEN | **empty** cuts what `49:` chose · `<kind>` cuts THAT kind once, without moving the selection | `opened a profile K hole at q,r` · `a window in the niche's back at q,r` · `opening refused — no wall here to open — stand against one` · `no niche to cut into — make one with N first` · `window refused — …` | **A** — row written 2026-08-18 at plan 22 `S2c`, and writing it is what the note at the top predicted: **the id had no row and the handler had its own copy of the gesture.** ⚠ **THE PAYLOAD IS A PROFILE, NOT A MATERIAL** — the units are the outline (`0` flat, `1` round, `2` pointed, `3` segmental, `4` an oculus) and the tens are the depth, so `11` is a round-headed NICHE and `21` an embrasure in one's back. `49:` chooses; this cuts. ⚠ **THE WHOLE GESTURE IS `hex_editor::session_open_kind`**; what stays here is the dirty mark and the sentences. ⛔ **IT WAS NOT, AND THE TWO DRIVERS BUILT DIFFERENT WORLDS** — the handler ran `open_ahead(…, DOOR_MAT, 1)` before `opening_make`, which is `D1a.2`'s one-edge width and its reversed order, so **six of the eight live scripts that open anything** diverged from `editor_run` and `house.keys` put its front door in a different wall segment with the same edge COUNT ([`probe/s2c`](../../probe/s2c/README.md)). ⚠ **AND ONE SENTENCE PER OUTCOME IS NEW HERE**: the old body sent the refusal AND `opened a profile …` over the top of it, because the success line ran after the embrasure branch too — a defect its own comment deferred to *the next commit*. ⚠ **`36:<kind>` DOES NOT MOVE THE SELECTION**, which is the contract it shares with `37:` and `38:` and which a bare `39:` cannot have |
| `54` | DECLARE | `<axis> <slot> <name…>` — the rest of the line, verbatim | `declared house 1 = longhouse wid=7 dep=5 opening=2 in region 0` · `declared house 1 cleared in region 0` · `declare refused — 'houses' is not a palette axis; say material, edge, item or house` · `declare refused — 'one' is not a slot number` · `declare refused — slot 0 is not writable — …` | **A** — added 2026-08-18, plan 22 `T1c`. ⛔ **IT IS THE FIRST WAY A PALETTE COULD BE WRITTEN AT ALL.** Measured at `T1b`: `hex_voxel::world_set_palette` had **zero production callers** — every one in the tree was a loft test — no driver has a `load` command, and this socket had no message for one. So plan 21's region mappings and plan 22's house types were both *readable by a gesture and writable only by a test*, through two plans, with every suite green. ⚠ **THE PAYLOAD IS THE LINE AND NOT FIELDS**, which is the one design decision in this row: a body carries spaces (`wid=5 dep=4`) **and** commas (`verbs=portcullis:opening,sally:hole`), so a `<axis>,<slot>,<body>` form would need a split-on-the-first-two rule in every reader — and `tools/script.mjs` would have to know the shape in order to BUILD one. Passing the rest of the line through means the driver decides nothing. ⚠ **THE WHOLE THING IS `hex_editor::world_declare`** — the parse, the four refusals and the sentence — and `editor_run`'s `declare` line calls the same function, which is `S2c`'s finding honoured in advance rather than after. What stays here is the sentence and one broadcast. ⚠ **AND THE BROADCAST IS NOT DECORATION**: a house type carries the opening an author who has chosen nothing will cut (`T1b`), so declaring one changes what `open_kind_here` answers and a subject line still showing the old kind is the `ps_status` literal again. ⚠ **The region is the AUTHOR's**, plan 21 `R2` — a declaration that always wrote region 0 would be global while the palette it lives in is not |
| `55` | VERB | `<verb>` — the name, verbatim | the GESTURE's own sentence · `no gesture for <verb>` · `refused: <why>` | **A** — added 2026-08-20, plan 22 `T1d`. ⛔ **UNTIL THIS, A VERB COULD NOT TRAVEL.** [EDITING_MODES](EDITING_MODES.md) says *the wire carries the verb, never the key*, and this socket carried neither: a MESSAGE ID per gesture, with `tools/script.mjs` holding a constant verb→id table. That is enough for the verbs a compiler saw — and a house type declares verbs no compiler ever will (`T1a.2`), so `verb portcullis` worked headless (where `editor_run` hands the word to `press_verb`) and answered *no verb 'portcullis'* over the wire. ⚠ **IT DOES NOT REPLACE THE DEDICATED IDS**: each of those handlers owns a SENTENCE that `K3` deliberately left in the server when it moved the gestures into the library (`fenced N edges, M stored outside`), and routing every verb through here would replace all of them with one generic line and redden the gates that read them. `script.mjs` consults its table first and reaches for `55:` only for a word it has never heard of. ⚠ **THE WHOLE THING IS `hex_editor::press_verb`**, alias resolution and all — the same call `editor_run` makes, so the two drivers are one body; `probe/s2c`'s `htverb` row compares their worlds byte for byte and a server that drops this message is caught there (measured: served opened 0 edges against the runner's 1). ⚠ **AND `K3a`'s FLOOR MOVED WITH THE AUTHORITY**: *a word the driver cannot send fails the run* used to fire on the table having no row; it now fires on this handler's `no gesture for` reply, so a typo still exits 101 while a declared verb is accepted — one authority, not two |
| `39` | SLAB / HOLE | `<0>` lays a slab · `<1\|2\|3>` cuts a void through the last one | `a slab T units thick at x,z — ceiling C, floor above F, clear K` · `a hole of kind N through the slab` · `hole refused — no slab to cut — lay a slab first` · `hole refused — the hole reaches past the slab's edge — offer O, residual R` | **A** — ⚠ **ONE ID, TWO GESTURES**, and this is the row's whole warning. `39:0` writes `es_slabs`; `39:1`…`39:3` write `es_holes` and **cannot run until a slab exists**. They are two verbs for that reason — `place`/`opening`'s relationship, not `seat`'s — and plan 22 `K3` bound them one slice apart: `Z` names `hole` and `X` names `slab`. ⚠ **AND `slab` IS SPELLED `39:0` IN `tools/script.mjs`, NOT BARE, ALTHOUGH BARE WOULD WORK** — the default below is exactly the slab, so `slab: '39:'` would be right by accident and would leave two bare rows on this id with nothing saying which the default served. ⚠ **A BARE `39:` IS A SLAB, NOT A CHOSEN HOLE.** The payload parses as `… ?? 0`, so the *empty means the one I chose* contract that `36:`, `37:` and `38:` share is **unavailable here** — a bare send lays a floor where the author asked for a void. Anyone adding a `SELECT_HOLE` must not reach for it. ⚠ **NOTHING ABOUT A HOLE REACHES THE STORE**: `es_holes` is session state and the mesher's only source, so a saved world and `w_tau` are **blind to this gesture by construction** — the sentence and the registry are the instruments (`probe/k2` check 2 cannot fail here, measured). ⚠ **The refusal used to say *press X first*** — a library naming a keystroke, which is wrong for a script, a remapped keyboard and a headless driver alike; a test was pinning it |
...
| `51` | SELECT_PART | `<name>` | `part 'door/leaf' chosen` · `nothing chosen — the placing verb builds a house again` · `part refused — …` | **A** — added 2026-08-14, plan 22 `B2f`. ⚠ **WHAT THE NEXT `32:` WILL PUT DOWN**, and `49:`'s twin one family over: `36:` cuts what `49:` chose, `32:` places what this chose. A **bare** `51:` clears it, so `place` builds the procedural house again — the way out is the same message. ⚠ **IT IS NOT `14:<roof>,<part>`, AND BOTH ARE KEPT.** That message names a part IN the placement, which is what a SCRIPT wants: one line saying exactly what to build, replayable with no state behind it. This is what a PERSON wants: choose once, then press the same key as always. Both end in the same `part_place`, so they cannot build different things — what differs is who remembers the name, and a script remembering it in the message is why `tools/scripts/*.keys` can be diffed. ⚠ **The choosing AND the refusal are `hex_editor::session_select_part`**; what stays in the server is the sentence. ⚠ **Whether the FILE exists is not asked here** — a well-formed name that is absent refuses when it is USED, because only the placing gesture is told where the library is, and the two drivers look in different places. ⚠ **Do not confuse it with `44:`, which is part MODE** — that opens a part for EDITING, and the client sent it here for one draft: both are "a message about a part", and the wrong one swaps the author's whole world for a door |
| `52` | SELECT_SEAT | `<kind>` | `seat K selected` · `selection refused — K is not a seat kind; nominal, so there is no nearest one` | **A** — added 2026-08-14, plan 22 `K3`. ⚠ **WHAT THE NEXT BARE `38:` WILL PUT IN A VOID**, and the third of exactly three selections: `49:` a profile, `51:` a part name, this a seat kind. Three ids because the VALUES are three kinds of thing; one shape because the question is one. ⚠ **IT IS NOT `49:` WITH A WIDER RANGE** — `0` and `1` are legal opening kinds AND legal seat kinds, so one message carrying both would be a value whose meaning depends on which gesture reads it next. ⚠ **`38:<kind>` does NOT move the selection**, exactly as `36:<kind>` does not, so a key means the same thing in every driver. ⚠ **It is what collapsed `Y` and `T` into one `seat` verb** — two keys on one message differing by the thing seated is `O`…`M`'s situation, and takes its answer. ⚠ **The choosing AND the refusal are `hex_editor::session_select_seat`**; what stays in the server is the sentence. Nominal, so a refusal offers nothing and the standing choice does not move |
| `53` | SELECT_ANNEX | `<kind>` | `annex K selected` · `selection refused — K is not an annex kind; nominal, so there is no nearest one` | **A** — added 2026-08-14, plan 22 `K3`. ⚠ **WHAT THE NEXT BARE `37:` WILL HANG**, and the fourth selection of exactly four (`49:` a profile, `51:` a part name, `52:` a seat kind). It collapsed `J`, `K` and `V` — three keys on one message differing by the thing hung — into one `annex` verb. ⚠ **ITS THIRD VALUE IS NOT A STORED KIND.** `0` and `1` are `AN_BOX` and `AN_DECK` and reach `annex_build` as exactly that; `2` (`hex_editor::ANX_CUPBOARD`) selects a different CONSTRUCTION — `annex_cupboard`, hosted on the last box rather than on a wall — and the annex it returns is itself an `AN_BOX`. Naming it `AN_CUPBOARD` would invite somebody to store it. ⚠ **`37:<kind>` does NOT move the selection.** ⚠ **The choosing AND the refusal are `hex_editor::session_select_annex`**; what stays in the server is the sentence and the dirty mark |
| `56` | AIM | `<material>` | `wall aimed N edges, heading H of 24 (aimed, residual R°), length L, from x0,z0 to x1,z1` · `aim refused — …` | **A** — added 2026-08-24, plan 24. ⚠ **THE FIRST MESSAGE THAT NEEDS THE AUTHOR'S YAW.** Every gesture before it reads x and z; `do_wall` says so in one line and hardcodes `yaw 0.0`. An aim reads the FACING and nothing about where the author walked, so the handler is given `wk.wk_yaw` — the walker's own, not a session pose. ⚠ **IT IS NOT `25:` WITH A FLAG, AND THAT IS ARITHMETIC RATHER THAN TASTE.** Both lay a wall run and they are DIFFERENT MAPS: `25:` traces, so `snap_run_d24` minimises one 2-D endpoint distance in which a direction error and a length error trade freely; this quantises the two axes separately, so the facing is honoured as a direction and the length is fitted afterwards. Measured, they agree on **77.7%** of aims ([`probe/m2p`](../../probe/m2p/README.md), control 11.6%) — [AUTHORING_MAP](AUTHORING_MAP.md) `X108`. ⚠ **AND IT NEVER TOUCHES THE `25:` DRAFT**, so an aim between a trace's two presses leaves that trace exactly where it was; mixing them inside one gesture is what `X108` forbids. ⚠ **NO LENGTH FIELD, DELIBERATELY** — the reach is `hex_editor::AIM_REACH`, because a key press has no magnitude and `input` has no continuous axis to read one from (BLUEPRINT §3.2, upstream). It grows a second field the day a stick supplies a push. ⚠ **Its refusal is REACHABLE where `25:`'s twin never fires**: `X111` — the twelve in-between directions have a period of 6.245 world units, so a short aim along one has no legal run at all |
| `57` | SELECT_REACH | `<units>` | `reach N selected` · `selection refused — aim reach N is above the maximum 24 (offer 24, residual R)` | **A** — added 2026-08-24, plan 24. ⚠ **THE FIFTH SELECTION AND THE FIRST THAT IS ORDINAL.** `49:` a profile, `51:` a part name, `52:` a seat kind, `53:` an annex kind — all **nominal**, so a bad value has no nearest one and the refusal offers nothing. A reach of 40 HAS a nearest one, so `fit_ordinal` names it: *offer 24, residual 16*. ⚠ **IT REFUSES RATHER THAN CLAMPING**, and that was checked rather than assumed — no caller of `fit_ordinal` in this tree applies `ft_offer`, so a selection that silently moved to the maximum would be the one gesture that does. ⚠ **WHOLE WORLD UNITS, NOT PERIODS, AND THE CHOICE IS FORCED**: a period is 1.0 or 1.732 units on the twelve exact headings and 6.245 on the twelve in-betweens (`X111`), so "3" would mean 3 units east and 18.7 at 15°. ⚠ **IT IS WHY `56:` CARRIES NO LENGTH, AND WHY IT MUST NOT GROW ONE**: when a stick supplies a push it feeds THIS message, or the editor has two authorities on how far an aim reaches and a script's choice is overruled by whatever the last frame read. ⚠ **The choosing AND the refusal are `hex_editor::session_select_reach`**; what stays in the server is the sentence |
| `58` | SELECT_WALL | `<slot>` | `wall N selected` · `selection refused — edge slot N is an opening, not a wall type; nominal, so there is no nearest one` · `selection refused — edge slot 3 is the fence material, not a wall type; the fence verb lays those` · `selection refused — edge slot N is not writable — 0 is absence and 255 is the last a byte names` | **A** — added 2026-08-26, plan 26 `B4e`. ⚠ **THE SIXTH SELECTION, AND IT NAMES A PALETTE SLOT RATHER THAN A KIND.** `@HB-X69` puts a wall's thickness in the EDGE palette — *"a uniform wall's 38 edges carry ONE distinct id and nothing else, so a per-edge thickness would be an eighth slot, which `L13` forbids"* — so **two walls of different thickness are two palette entries**, and the only gesture the store can carry is *choose a type*. It pairs with `54:` (`declare edge <slot> <name> body=… thick=…`): the declaration says what an id means, this says which id to stamp. ⚠ **IT READS FOR `25:`, `56:` AND `verb wall` ALIKE**, from `hex_editor::session_wall_mat` — one reader, so the three verbs cannot come to disagree. ⚠ **AND IT DOES NOT CHECK THAT THE WORLD DECLARED THE SLOT**, deliberately: `51:`'s rule — whether a slot is a legal wall type is a fact about the NUMBER, whether anything is declared in it is a fact about a WORLD, and a selection outlives the world loaded next. The plan view is what shows the difference, and it shows it **loudly** — an undeclared byte keeps the magenta. ⚠ **THE VOCABULARY'S OWN BYTES ARE REFUSED BY NAME**: `2` a door, `3` a fence, `4` a window are real edge materials and none of them is a wall type, so choosing one would quietly turn the wall verb into a different gesture. Nominal, nothing offered. ⚠ **`24:<dir>,<mat>` IS NOT ITS TWIN WITH A MEMORY** — that writes ONE edge at a coordinate (the atomic half a fixture uses); this is what a person chooses once and then presses the key they always press |
