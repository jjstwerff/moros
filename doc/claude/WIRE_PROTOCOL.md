...
contract the three ids beside it share. **Ten left** — `36` was written at `S2c` and `54` was born with a row at `T1c` (both 2026-08-18), for the reason this note gives: its absence is what let the server keep a second body of the gesture with nothing pointing at it. They arrived after the
