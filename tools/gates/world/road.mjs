...
// The live chunk meshes as (id, floats) — `X:` retirements subtracted, so a chunk the
// server dropped cannot still contribute a height.
const liveChunks = () => {
  const out = [];
...
for (let x = 2; x <= 26; x += 2) await send(g, `7:${x},0,0`, ['placed']);
