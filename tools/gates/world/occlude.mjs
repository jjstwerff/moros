...
// (y 3.424 → 3.899) rather than pulling in, and both are correct. A gate written
...
const placeAt = async (q, r) => {
  ...
  await ack('placed');
  await wait(1500);          // let both settle — they ease rather than snap
  return cam();
};
...
ws.onmessage = async (e) => {
  ...
  if (t === 'C' && !st) { st = 1;
    ...
    const ok = meaningful && fenceIgnored && wallGaveWay;
    console.log(JSON.stringify({ ...out, wallTop, meaningful, fenceIgnored, outside, wallGaveWay, ok }));
