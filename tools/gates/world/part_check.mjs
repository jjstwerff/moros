...
// ⚠ THE FEW GESTURES THAT ANSWER NOTHING OF THEIR OWN. Named for what it is:
// there is no evidence to wait for, so the window is the instrument.
const step = async (msg, ms = 1200) => {
  const before = g.says.length;
  g.ws.send(msg);
  await absenceWindow(ms, 'this gesture acknowledges nothing, so there is no line to wait for');
...
const refused = await askOne('8:prop/plinth', 'part save refused');
...
const refusedAgain = await askOne('8:prop/plinth', 'part save refused');
check(refusedAgain !== '', 'the refusal stands with an edit pending');
const closed = await askOne('44:', "part 'prop/shrine'");
...
const sound = await askOne('8:prop/newshrine', 'part ');
...
const cellSave = await askOne('8:', 'part ');
...
const meshRefused = await askOne('8:', 'part save refused');
