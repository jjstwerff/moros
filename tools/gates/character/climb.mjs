...
const posOf = () => { const t = trace(); return t.length ? t[t.length - 1] : null; };
// ⚠ `T:` IS BROADCAST ONLY FROM INSIDE `if moved`, so once the character stands still
// there is no next one and waiting for one times out. `2:<aspect>,` sets `moved`, so
// the tick that follows broadcasts a fresh transform — a position can be REQUESTED.
// That is what makes the reads below barriers rather than guesses.
const freshPos = async (limitMs = 8000) => {
  const before = trace().length;
  g.ws.send('2:1.5,');
  const got = await until(() => trace().length !== before, 'no fresh transform', limitMs);
  return got ? posOf() : null;
};
// Two requested reads that agree is a fixed point on the observable. There is no
// deceleration to wait out — `if fwd != 0.0` in the server means key-up stops the walk
// in the same tick — so this converges at once and says so if it does not.
...
const yAtDistance = (i0, cum, d) => {
  ...
  for (let k = 1; k < cum.length; k++) {
    if (cum[k] >= d) {
      ...
      const ya = tr[i0 + k - 1][1], yb = tr[i0 + k][1];
      return ya + (yb - ya) * f;
    }
  }
...
// A raise applies in full before the next message is read and this gate reads no mesh,
// so the ordered wire is the whole barrier.
...
// Hold W and step forward one TRANSFORM at a time — the server's own tick, not a 50 ms
// timer of ours — until the ground covered reaches TARGET. The clock, not the walk:
// `34:8` consumes the same FIXED ticks eight times faster (STATE.md: three rates,
...
cum = pathAt(i0);
// Concluded from the path, not from where the loop broke — a loop that gave up for any
// other reason must not be able to claim the ground was covered.
const reached = cum[cum.length - 1] >= TARGET;

const yT = reached ? yAtDistance(i0, cum, TARGET) : null;
const climbed = yT === null ? null : yT - y0;
// Monotone over the WHOLE ascent, not the last 30 samples — the claim is that walking
// into a slope never drops, and the walk is the thing being claimed of.
...
for (let i = i0 + 2; i < i0 + cum.length; i += 2) {
  const a = tr2[decIdx[decIdx.length - 1]], b = tr2[i];
...
for (let k = 1; k < dec.length; k++) {
  if (dec[k] >= TARGET) {
    ...
    const ya = tr2[decIdx[k - 1]][1], yb = tr2[decIdx[k]][1];
    yHalf = ya + (yb - ya) * f;
    break;
  }
}
...
// ⚠ 0.25, NOT 0.4, AND THE DIFFERENCE IS A BUG THIS GATE USED TO CONTAIN. `climbed` is
// `yAtTarget - startY`, and `yAtTarget` is unchanged at 0.619 — only the BASELINE
// moved. `startY` used to read 0 because the feet were `start.y`, set once and
// refreshed only on a MOVE; the four raises lift the ground under the standing
// character by 1 height unit, which nothing noticed until it walked. Measured on
// correct code: **0.369**, and 0.25 keeps the same 1.5x margin the old pair had.
